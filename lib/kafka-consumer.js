 ...
 * is fetched.
 */
KafkaConsumer.prototype.consume = function(number, partition, cb) {
  var timeoutMs = this._consumeTimeout !== undefined ? this._consumeTimeout : DEFAULT_CONSUME_TIME_OUT;
  var self = this;

  if ((number && typeof number === 'number') && (typeof partition === 'number') || ((number && typeof number === 'number') && (typeof partition === 'number') && cb)) {
    ...
    this._consumeNumOfPartition(timeoutMs, number, partition, cb);
 ...
 * @see consume
 */
KafkaConsumer.prototype._consumeNumOfPartition = function(timeoutMs, numMessages, partition, cb) {
  var self = this;

  this._client.consume(timeoutMs, numMessages, partition, function(err, messages, eofEvents) {
    if (err) {
      err = LibrdKafkaError.create(err);
      if (cb) {
        cb(err);
