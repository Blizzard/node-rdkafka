...
KafkaConsumer.prototype._consumeLoop = function(timeoutMs, topics, cb) {
  ...
  try {
    // Only subscribe if there are no existing assignments or subscriptions,
    // keep the call to subscribe not to break the API
    if (this.assignments().length === 0 && this.subscription().length === 0) {
