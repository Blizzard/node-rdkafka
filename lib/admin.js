 ...
 * the upstream broker is connected to. Instead, making an action will validate that.
 *
 ...
 *
 * @param {NewTopic} topic - Topic partitions with offsets to store offsets for.
...
AdminClient.prototype.createTopic = function(topic, timeout, cb) {
  ...
  this._client.createTopic(topic, function(err) {
    if (err) {
      if (cb) {
